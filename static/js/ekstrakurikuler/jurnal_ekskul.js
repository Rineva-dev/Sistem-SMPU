...
document.getElementById('modalLibur').addEventListener('show.bs.modal', async () => {
    ...
    const elemenPeriode = document.getElementById('periode-ekskul');

    const tglMulai = ubahTanggal(elemenPeriode.dataset.mulai);
    const tglSelesai = ubahTanggal(elemenPeriode.dataset.selesai);
    const ekskulId = elemenPeriode.dataset.ekskulId;

    if (!tglMulai || !tglSelesai || isNaN(tglMulai.getTime()) || isNaN(tglSelesai.getTime())) {
        wadah.innerHTML = `<div class="text-danger">Tentukan dulu periode kegiatan pada data ekskul!</div>`;
        return;
    }
    ...
    let sudahLibur = [];
    try {
        const res = await fetch(`/data-ekskul/libur/daftar?ekskul_id=${ekskulId}`);
...
document.getElementById('formLibur').addEventListener('submit', async e => {
    ...
    const ekskulId = fd.get('ekskul_id');
    ...
    try {
        const res = await fetch(e.target.action, {
            ...
            body: JSON.stringify({ekskul_id: ekskulId, daftar: dataSimpan})
        });
